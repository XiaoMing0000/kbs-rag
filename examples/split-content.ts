...
`;
// 固定分块
const fixedSizeSegments = ChunkingStrategy.fixedSize(content, { chunkSize: 30, overlap: 10 });
console.log(fixedSizeSegments);

// 根据语句长度进行分块
const sentencesSegments = ChunkingStrategy.sentences(content, { chunkSize: 100 });
console.log(sentencesSegments);

// 递归分块
const textSegments = ChunkingStrategy.recursive(content, { chunkSize: 100, separators: ['\n\n', '\n', ' '], overlap: 0 });
console.log(textSegments);

// 递归分块，分割 markdown
const markdownSegments = ChunkingStrategy.recursive(content, { chunkSize: 100, separators: ['\n# ', '\n## ', '\n### '], overlap: 0 });
console.log(markdownSegments);
